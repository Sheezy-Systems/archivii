use std::error::Error;
use std::{env, fmt, fs, thread, time::Duration};
...
fn main() {
    ...
    for page in (config.start)..(config.start + config.limit - 1) {
...
struct SchoologyUsers {
    users: HashMap<String, SchoologyUser>
}

#[derive(Serialize, Deserialize)]
struct SchoologyRealm {
    // todo: id, name, banner, information, category, members
    posts: Vec<SchoologyPost>
}
...
struct CommentsResponse {
    comments: String,
    count: String, // Schoology stores the amount of comments as a string.
}
